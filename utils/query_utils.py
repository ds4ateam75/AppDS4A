...
class Carga_query:
    ...
    def __init__(self, update_map=True):
        self.update_map = update_map
        if self.update_map is True:
            self.query = "SELECT ARCO, SUM(CARGA) AS CARGA FROM CONSULTA "
        else:
            self.query = "SELECT HORA, SUM(CARGA) AS CARGA FROM CONSULTA "
    ...
    def restart_query(self, update_map=True):
        self.update_map = update_map
        if self.update_map is True:
            self.query = "SELECT ARCO, SUM(CARGA) AS CARGA FROM CONSULTA "
        else:
